...
const SCHEMA = {
  ...
  stock:    { key: 'id', cols: ['id', 'vehicle', 'part_name', 'part_no', 'source', 'category', 'unit_price', 'cost_price', 'init_qty', 'notes'], num: ['unit_price', 'cost_price', 'init_qty'] },
  suppliers:{ key: 'id', cols: ['id', 'name', 'contact', 'address', 'status', 'created_at'], num: [] },
  sales:    { key: 'id', cols: ['id', 'sale_date', 'branch', 'staff_email', 'invoice_no', 'vehicle', 'part_name', 'part_no', 'qty', 'unit_price', 'sale_value', 'cost_price', 'gross_profit', 'stock_before', 'stock_after', 'remarks', 'created_at', 'customer_name', 'vehicle_no', 'chassis_no'], num: ['qty', 'unit_price', 'sale_value', 'cost_price', 'gross_profit', 'stock_before', 'stock_after'] },
...
CREATE TABLE IF NOT EXISTS stock (
  ...
  source TEXT DEFAULT '', category TEXT, unit_price NUMERIC DEFAULT 0, cost_price NUMERIC DEFAULT 0,
  init_qty INTEGER DEFAULT 0, notes TEXT
);
CREATE TABLE IF NOT EXISTS suppliers (
  id TEXT PRIMARY KEY, name TEXT NOT NULL UNIQUE, contact TEXT, address TEXT,
...
CREATE TABLE IF NOT EXISTS sales (
  ...
  remarks TEXT, created_at TEXT, customer_name TEXT, vehicle_no TEXT, chassis_no TEXT
);
...
-- A sale invoice may hold several parts (multi-line). Uniqueness is per part within
-- an invoice, so the same invoice can list different parts but not duplicate one.
DROP INDEX IF EXISTS uq_sales_invoice_branch;
CREATE UNIQUE INDEX IF NOT EXISTS uq_sales_invoice_branch_part ON sales(branch, invoice_no, part_name, part_no) WHERE invoice_no IS NOT NULL AND invoice_no <> '';
-- Add columns to pre-existing tables (no-op when already present).
ALTER TABLE sales ADD COLUMN IF NOT EXISTS chassis_no TEXT;
ALTER TABLE stock ADD COLUMN IF NOT EXISTS category TEXT;
-- Stock identity now includes vehicle: replace the old (part_name, part_no) unique.
ALTER TABLE stock DROP CONSTRAINT IF EXISTS stock_part_name_part_no_key;
CREATE UNIQUE INDEX IF NOT EXISTS uq_stock_identity ON stock (part_name, part_no, vehicle);
...
const normVeh = v => String(v == null ? '' : v).trim();
const partKey = (name, pno, vehicle) => String(name == null ? '' : name).trim() + '' + normPno(pno) + '' + normVeh(vehicle);
...
function findStock(name, partNo, vehicle) { const k = partKey(name, partNo, vehicle); const r = data.stock.find(s => partKey(s.part_name, s.part_no, s.vehicle) === k); return r ? { ...r } : null; }
...
function qtyMaps(branch) {
  ...
  for (const i of data.inward) {
    ...
    const k = partKey(i.part_name, i.part_no, i.vehicle);
    inMap.set(k, n(inMap.get(k)) + n(i.qty));
  }
  for (const s of data.sales) {
    if (branch && branch !== 'ALL' && s.branch !== branch) continue;
    const k = partKey(s.part_name, s.part_no, s.vehicle);
    outMap.set(k, n(outMap.get(k)) + n(s.qty));
  }
  if (branch && branch !== 'ALL') {
    for (const t of data.transfers) {
      const k = partKey(t.part_name, t.part_no, t.vehicle);
...
function currentQty(partName, partNo, vehicle, branch) {
  const k = partKey(partName, partNo, vehicle);
  const s = data.stock.find(x => partKey(x.part_name, x.part_no, x.vehicle) === k);
  if (!s) return null;
...
function stockWithQty(branch) {
  const { inMap, outMap } = qtyMaps(branch);
  return data.stock
    .map(s => { const k = partKey(s.part_name, s.part_no, s.vehicle); return { ...s, current_qty: n(s.init_qty) + n(inMap.get(k)) - n(outMap.get(k)) }; })
...
async function init() {
  ...
  // Seed the supplier master (first boot) from any suppliers already named on inward rows.
  if (data.suppliers.length === 0) {
    const names = [...new Set(data.inward.map(i => (i.supplier || '').trim()).filter(Boolean))].sort();
    if (names.length) {
      let seq = 0;
      await bulkInsert('suppliers', names.map(name => ({
        id: 'SUP' + String(++seq).padStart(3, '0'), name, contact: '', address: '', status: 'Active',
      })));
    }
  }
